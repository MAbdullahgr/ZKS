...
test.describe("Brands — management", () => {
  ...
  test("loads /brands and shows the brand list", async ({ page }) => {
    await login(page);
  ...
  test("can open the add-brand form", async ({ page }) => {
    await login(page);
    await selectStore(page);
    await page.goto("/brands");
    await page.waitForLoadState("networkidle", { timeout: 20_000 }).catch(() => {});

    const addBtn = page.getByRole("button", { name: /add brand|new brand/i }).first();
